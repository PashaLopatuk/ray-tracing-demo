...
bool rayIntersectBVH(Ray ray, out RayHitResult nearest) {
    int stack[BV_MAX_STACK_SIZE];
    RayHitResult current;
    vec4 textureColor;
    int boundingVolumeIdx;
    int f0_idx, f1_idx, lt_idx, rt_idx;
    int topPointer;
    nearest.t = MAX_FLT;
    for (int obj_idx = 0; obj_idx < u_num_objects; ++obj_idx) {
        stack[0] = 0;
        topPointer = 0;
        while (topPointer > -1) {
            boundingVolumeIdx = stack[topPointer--];
            if (rayIntersectBV(ray, texelFetch(u_aabb_sampler, ivec3(BV_MIN_BOUNDS_INDEX, boundingVolumeIdx, obj_idx), 0).xyz,
            texelFetch(u_aabb_sampler, ivec3(BV_MAX_BOUNDS_INDEX, boundingVolumeIdx, obj_idx), 0).xyz
            )) {
                textureColor = texelFetch(u_aabb_sampler, ivec3(BV_PAYLOAD_INDEX, boundingVolumeIdx, obj_idx), 0);
                rt_idx = int(textureColor.g);
                lt_idx = int(textureColor.r);
                f1_idx = int(textureColor.a);
                f0_idx = int(textureColor.b);
                if (f0_idx != -1 && rayIntersectFace(ray, f0_idx, obj_idx, current) && current.t < nearest.t) {
                    nearest = current;
                }
                if (rt_idx != -1) { stack[++topPointer] = rt_idx; }
                if (lt_idx != -1) { stack[++topPointer] = lt_idx; }
                if (f1_idx != -1 && rayIntersectFace(ray, f1_idx, obj_idx, current) && current.t < nearest.t) {
                    nearest = current;
                }
            }
        }
    ...
    if (nearest.t != MAX_FLT) {
        if (u_shadingMethod == FLAT_SHADING) {
...
void main() {
    ...
    // transform ray target pos on image plane from view space to world space
    vec4 rayTarget = u_eye_to_world * vec4(
    v_eye_to_x + rand(),
    u_eye_to_image,
    v_eye_to_z + rand(),
    1.0
    ...
    if (u_render_pass == 1) {
        o_color = vec4(pathTrace(ray), 1.0);
    } else {
